...
import com.lzbay.pm.base.common.utils.CustomIpUtil;
...
public class Ip2RegionListener implements ApplicationListener<ApplicationEnvironmentPreparedEvent> {
    ...
    public void onApplicationEvent(ApplicationEnvironmentPreparedEvent applicationEvent) {
        ...
        String logDirectoryPath = environment.getProperty(ConfigConst.LOG_DIRECTORY);
        if (logDirectoryPath == null) {
            throw new ExceptionInInitializerError("环境变量为空：" + ConfigConst.LOG_DIRECTORY);
        }
        System.setProperty(ConfigConst.LOG_DIRECTORY, logDirectoryPath);
        ...
        try {
            FileUtil.copyFile(new ClassPathResource(IP_FILE_NAME).getInputStream(), tempFile,
                    StandardCopyOption.REPLACE_EXISTING);

            // 2、初始化
            CustomIpUtil.init(tempFilePath);
